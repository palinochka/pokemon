// GENERATED CODE - DO NOT MODIFY BY HAND

part of 'details_store.dart';

// **************************************************************************
// StoreGenerator
// **************************************************************************

// ignore_for_file: non_constant_identifier_names, unnecessary_brace_in_string_interps, unnecessary_lambdas, prefer_expression_function_bodies, lines_longer_than_80_chars, avoid_as, avoid_annotating_with_dynamic, no_leading_underscores_for_local_identifiers

mixin _$DetailsStore on _DetailsStore, Store {
  Computed<PokemonModel?>? _$modelComputed;

  @override
  PokemonModel? get model =>
      (_$modelComputed ??= Computed<PokemonModel?>(() => super.model,
              name: '_DetailsStore.model'))
          .value;

  late final _$_modelAtom =
      Atom(name: '_DetailsStore._model', context: context);

  @override
  PokemonModel? get _model {
    _$_modelAtom.reportRead();
    return super._model;
  }

  @override
  set _model(PokemonModel? value) {
    _$_modelAtom.reportWrite(value, super._model, () {
      super._model = value;
    });
  }

  late final _$viewIsReadyAsyncAction =
      AsyncAction('_DetailsStore.viewIsReady', context: context);

  @override
  Future<void> viewIsReady(String url) {
    return _$viewIsReadyAsyncAction.run(() => super.viewIsReady(url));
  }

  @override
  String toString() {
    return '''
model: ${model}
    ''';
  }
}
